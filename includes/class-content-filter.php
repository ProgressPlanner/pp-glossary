...
class Content_Filter {
	...
	public static function filter_content( $content ): string {
		...
		$glossary_entries = inline_glossary_get_linkable_entries();
	...
	private static function replace_first_occurrence( $content, $entry ): string {
		...
		$parts            = inline_glossary_split_by_excluded_tags( $content, $excluded_tags );
	...
	private static function create_term_button( $term, $unique_id, $popover_id ): string {
		...
		return sprintf(
			'<dfn id="%s" class="inline-glossary-term" style="anchor-name: %s;"><span data-glossary-popover="%s" tabindex="0" role="button" aria-expanded="false">%s</span></dfn>',
			esc_attr( $unique_id ),
	...
	private static function create_popover( $entry, $unique_id, $popover_id ): string {
		...
		$sr_text       = $has_read_more
			...
			? sprintf( __( 'Definition of %s. Link to full glossary entry follows the description.', 'inline-glossary' ), $entry['title'] )
			/* translators: %s: glossary term title */
			: sprintf( __( 'Definition of %s.', 'inline-glossary' ), $entry['title'] );

		$popover_html .= sprintf( '<span class="inline-glossary-sr-only">%s</span>', esc_html( $sr_text ) );
		...
		if ( $has_read_more ) {
			...
			$popover_html .= sprintf(
				...
				esc_url( $full_url ),
				/* translators: %s: glossary term title */
				esc_attr( sprintf( __( 'Read more about %s on the glossary page', 'inline-glossary' ), $entry['title'] ) ),
				esc_html__( 'Read more about', 'inline-glossary' ),
				esc_html( $title )
			);
		}
